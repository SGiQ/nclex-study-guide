...
with open('NEW_QUIZ_EP14_PART_A.json', 'r', encoding='utf-8') as f:
    quiz_a = json.load(f)

with open('NEW_QUIZ_EP14_PART_B.json', 'r', encoding='utf-8') as f:
    quiz_b = json.load(f)

# Unlabel old quizzes
found_old = False
for quiz in quizzes:
    if quiz.get('episodeId') == 14:
        print(f"Unlabeling old quiz: {quiz['title']} (ID: {quiz['id']})")
        del quiz['episodeId']
        if "Part 3" in quiz['title']:
             quiz['title'] = "Physiological Adaptation Part 3 (Unassigned)"
        found_old = True

if not found_old:
    print("No existing Episode 14 quiz found.")
...
with open('NEW_FLASHCARDS_EP14.json', 'r', encoding='utf-8') as f:
    new_cards = json.load(f)

existing_index = -1
for i, deck in enumerate(cards_list):
    if deck.get('episodeId') == 14:
        existing_index = i
        break

if existing_index != -1:
    print(f"Replacing existing Ep 14 flashcards (Index {existing_index})")
    cards_list[existing_index] = new_cards
else:
    print("Adding new flashcards for Ep 14")
