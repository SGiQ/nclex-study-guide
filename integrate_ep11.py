...
with open('NEW_QUIZ_EP11_PART_A.json', 'r', encoding='utf-8') as f:
    quiz_a = json.load(f)

with open('NEW_QUIZ_EP11_PART_B.json', 'r', encoding='utf-8') as f:
    quiz_b = json.load(f)

# Unlabel old quizzes
found_old = False
for quiz in quizzes:
    if quiz.get('episodeId') == 11:
        print(f"Unlabeling old quiz: {quiz['title']} (ID: {quiz['id']})")
        del quiz['episodeId']
        if "Part 2" in quiz['title']:
             quiz['title'] = "Risk Reduction Part 2 (Unassigned)"
        found_old = True

if not found_old:
    print("No existing Episode 11 quiz found.")
...
with open('NEW_FLASHCARDS_EP11.json', 'r', encoding='utf-8') as f:
    new_cards = json.load(f)

existing_index = -1
for i, deck in enumerate(cards_list):
    if deck.get('episodeId') == 11:
        existing_index = i
        break

if existing_index != -1:
    print(f"Replacing existing Ep 11 flashcards (Index {existing_index})")
    cards_list[existing_index] = new_cards
else:
    print("Adding new flashcards for Ep 11")
